...
@Service
public class SplitFarePricingService {
    ...
    public RideConfirmationDetails bookRide(RideRequest rideRequest) {
        ...
        rideDriverMap.getMap().forEach((driver, rideServiceProviderContext) -> {
                    if (rideServiceProviderContext.getRideType() == RideType.SPLIT &&
        ...
        tripOptions.sort((p1, p2) -> p1.getRideServiceProviderContext().getSplitInfo().getSrNo() > p2.getRideServiceProviderContext().getSplitInfo().getSrNo() ? 1 : -1);


        return RideConfirmationDetails.builder()
                .bookingId(bookingId.toString())
                .status(true)
                .splitBooking(SplitBooking.builder().tripOptions(tripOptions).build())
                .build();
    }
}
